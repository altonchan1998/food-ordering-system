...
public class PaymentDomainServiceImpl implements PaymentDomainService {
    ...
    public PaymentEvent validateAndInitiatePayment(
        ...
        if (failureMessages.isEmpty()) {
            ...
            return new PaymentCompletedEvent(payment, ZonedDateTime.now(ZoneOffset.UTC), paymentCompletedEventDomainEventPublisher);
        } else {
    ...
    public PaymentEvent validateAndCancelPayment(
        ...
        payment.validatePayment(failureMessages);
        addCreditHistory(payment, creditEntry);
        updateCreditHistory(payment, creditHistories, TransactionType.CREDIT);

        if (failureMessages.isEmpty()) {
            log.info("Payment is cancelled for order id: {}", payment.getOrderId().getValue());
            payment.updateStatus(PaymentStatus.CANCELLED);
            return new PaymentCancelledEvent(payment, ZonedDateTime.now(ZoneOffset.UTC), paymentCancelledEventDomainEventPublisher);
        } else {
    ...
    private void addCreditHistory(Payment payment, CreditEntry creditEntry) {
